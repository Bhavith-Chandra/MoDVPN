...
pub mod proto {
    tonic::include_proto!("modvpn.controller");
}
...
pub async fn new_grpc_client() -> Result<ControllerServiceClient, ControllerError> {
    let ipc_path = modvpn_config::config().socket_path();
    ...
    P: Auth + 'static,
{
    ...
    let socket_path = modvpn
    
    
    _config::config().socket_path();
