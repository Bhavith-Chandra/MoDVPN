...
impl EventForwarder {
    ...
    async fn event_watch_loop(app_handle: AppHandle) {
        loop {
            match modvpn_controller::new_grpc_client().await {
                Ok(mut client) => {
                            ...
                            while let Some(event) = stream.next().await {
                                if let Ok(event) = event {
                                    if let Some(event) = event.event {
                                        match event {
                                            modvpn_controller::proto::daemon_event::Event::VpnStatus(vpn_status) => {
                                                let vpn_status: modvpn_types::vpn_session::VpnStatus = vpn_status.into();
                                                update_app_state(app_handle.clone(), vpn_status.clone()).await;
                                                let _ = app_handle.emit_all("vpn_status", vpn_status);
                                            },
                                            modvpn_controller::proto::daemon_event::Event::Notification(notification) => {
