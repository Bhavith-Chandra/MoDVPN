...
const CONFIG_FILENAME: &str = "modvpn.conf.toml";
...
pub fn config() -> &'static Config {
    ...
    let config_dir = CONFIG_DIR.get_or_init(|| {
        #[cfg(unix)]
        return PathBuf::from("/etc/modvpn");
        #[cfg(windows)]
        {
            return program_data_path.join("modvpn");
        }
    });

    let _ = LOG_DIR.get_or_init(|| {
        #[cfg(unix)]
        return PathBuf::from("/var/log/modvpn");
        #[cfg(windows)]
        return program_data_path.join("modvpn").join("log");
    });

    let _ = SOCKET_PATH.get_or_init(|| {
        #[cfg(unix)]
        return PathBuf::from("/var/run/modvpn.sock");
        #[cfg(windows)]
        return PathBuf::from("//./pipe/modvpn");
    });
...
impl Default for Config {
    fn default() -> Self {
        Self {
            ...
            grpc_api_host_port: "grpcs://api.modvpn.app:44444".into(),
            socket_path: SOCKET_PATH.get().unwrap().into(),
            daemon_log_filename: "modvpn-daemon.log".into(),
            // IP of api.modvpn.app
...
impl Config {
    ...
    pub fn db_url(&self) -> String {
        format!("sqlite://{}/modvpn.db?mode=rwc", self.db_dir().display())
    }
    ...
    pub fn license_file_path(&self) -> PathBuf {
        ...
        #[cfg(target_os = "linux")]
        return PathBuf::from_str("/opt/modvpn/modvpn-oss-licenses.html").unwrap();
        #[cfg(target_os = "macos")]
        return PathBuf::from_str(
            "/Applications/modvpn.app/Contents/Resources/modvpn-oss-licenses.html",
        )
        .unwrap();
        ...
        .join("modvpn")
        .join("modvpn-oss-licenses.html");
    }

    pub fn icon_path(&self) -> &'static str {
        #[cfg(target_os = "linux")]
        return "/usr/share/icons/hicolor/32x32/apps/modvpn.png";
        #[cfg(target_os = "macos")]
        return "/Applications/modvpn.app/Contents/Resources/icon.icns";
        #[cfg(target_os = "windows")]
        return "";
    }
}
