...
impl ControllerServerAndEventBroadcaster {
    async fn notify(&self, event: modvpn_controller::proto::DaemonEvent) {
...
impl EventListener for ControllerServerAndEventBroadcaster {
    async fn send_vpn_status(&self, status: modvpn_types::vpn_session::VpnStatus) {
        tracing::debug!("notifying new vpn status");
        self.notify(modvpn_controller::proto::DaemonEvent {
            event: Some(modvpn_controller::proto::daemon_event::Event::VpnStatus(
                modvpn_controller::proto::VpnStatus::from(status),
            )),
        })
        .await
    }
    ...
    async fn send_notification(&self, notification: Notification) {
        ...
        self.notify(modvpn_controller::proto::DaemonEvent {
            event: Some(modvpn_controller::proto::daemon_event::Event::Notification(
                modvpn_controller::proto::Notification::from(notification),
            )),
        })
        .await
    }
...
pub type VpnStatusListenerSender =
    tokio::sync::mpsc::UnboundedSender<Result<modvpn_controller::proto::VpnStatus, Status>>;
pub type VpnStatusListenerReceiver =
    UnboundedReceiverStream<Result<modvpn_controller::proto::VpnStatus, Status>>;

pub type DaemonEventsListenerSender =
    tokio::sync::mpsc::UnboundedSender<Result<modvpn_controller::proto::DaemonEvent, Status>>;

pub type DaemonEventsListenerReceiver =
    UnboundedReceiverStream<Result<modvpn_controller::proto::DaemonEvent, Status>>;
...
impl ControllerService for ControllerServiceImpl {
    ...
    async fn connect_vpn(
        &self,
        req: Request<modvpn_controller::proto::Location>,
        ...
        self.wait_for_result(rx)
            .await?
            .map(modvpn_controller::proto::VpnStatus::from)
            .map(Response::new)
            .map_err(map_daemon_error)
    }
    ...
    async fn get_notifications(&self, _: Request<()>) -> ServiceResult<Notifications> {
        ...
        self.wait_for_result(rx)
            .await?
            .map(modvpn_controller::proto::Notifications::from)
            .map(Response::new)
            .map_err(map_daemon_error)
    }
