...
use modvpn_server::{auth::TokenProvider, ServerApi};
use modvpn_types::{
    location::Location,
    modvpn_server::{
...
impl VpnSessionHandler {
    ...
    pub async fn new_session(
        &self,
        new_session: modvpn_types::modvpn_server::NewSession,
    ...
    pub async fn end_session(
        &self,
        end_session: modvpn_types::modvpn_server::EndSession,
...
impl<P: TokenProvider + 'static> VpnSessionService<P> {
    ...
    async fn on_new_session_inner(
        ...
        let mut modvpn_service = ServerApi::new(token_provider).await?;
        Ok(modvpn_service.new_session(new_session).await?)
    }
    ...
    async fn on_end_session_inner(
        ...
        backoff::future::retry(bf, || async {
            let mut modvpn_service = ServerApi::new(token_provider.clone())
                .await
    ...
    async fn on_client_connected_inner(
        ...
        let mut modvpn_service = ServerApi::new(token_provider).await?;
        Ok(modvpn_service.client_connected(client_connected).await?)
    }
    ...
    async fn on_list_locations_inner(
        ...
        let mut modvpn_service = ServerApi::new(token_provider).await?;
        let locations = modvpn_service.list_locations().await?;
        Ok(locations)
    }
