...
use modvpn_server::auth::TokenProvider;
use modvpn_server::ServerApi;
use modvpn_types::modvpn_server::{VpnSessionStatus, VpnSessionStatusRequest};
...
impl<P: TokenProvider + 'static> Watcher<P> {
    ...
    async fn watch_ended(&mut self) -> bool {
        ...
        let modvpn_service = ServerApi::new(token_provider).await;
        match modvpn_service {
            Ok(mut modvpn_service) => {
                match modvpn_service
                    ...
                    .await
                {
                    Ok(vpn_session_status) => {
                        ...
                        match vpn_session_status {
                            modvpn_types::modvpn_server::VpnSessionStatus::Failed(_)
                            | modvpn_types::modvpn_server::VpnSessionStatus::ServerReady(_)
                            | modvpn_types::modvpn_server::VpnSessionStatus::ClientConnected(_)
                            | modvpn_types::modvpn_server::VpnSessionStatus::Ended(_) => {
                                tracing::info!("watcher end state received: {vpn_session_status}");
                                return true;
                            }
                            modvpn_types::modvpn_server::VpnSessionStatus::ServerCreated(_)
                            | modvpn_types::modvpn_server::VpnSessionStatus::ServerRunning(_)
                            | modvpn_types::modvpn_server::VpnSessionStatus::Accepted(_) => {}
                        };
                    }
                    Err(err) => {
            ...
            Err(err) => {
                ...
                tracing::error!("failed to connect to modvpn service from watcher for {vpn_session_status_request}: {err}");
            }
        }
        false
    }
