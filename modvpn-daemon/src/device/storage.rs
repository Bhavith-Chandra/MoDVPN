...
impl DeviceStorage {
    ...
    pub async fn init(&self) -> Result<DeviceDetails, String> {
        ...
        let device = modvpn_entity::device::Entity::find()
            .one(&self.db)
            .await
        ...
        // save to DB
        let device: modvpn_entity::device::Model = device_details.clone().into();
        let device: modvpn_entity::device::ActiveModel = device.into();
    ...
    pub async fn reinitialize(&self, reason: &str) -> Result<(), DbErr> {
        ...
        let _ = modvpn_entity::device::Entity::delete_many()
            .exec(&self.db)
            .await?;
    ...
    pub async fn get_device_unique_id(&self) -> Result<Uuid, DbErr> {
        let id: (String,) = modvpn_entity::device::Entity::find()
            .select_only()
            .column(modvpn_entity::device::Column::UniqueId)
            .into_tuple()
            .one(&self.db)
            .await?
            .unwrap();
    ...
    pub async fn get_device(&self) -> Result<Option<DeviceDetails>, DbErr> {
        let device = modvpn_entity::device::Entity::find().one(&self.db).await?;
    ...
    pub async fn update_ipv4_address(
        ...
        let device = modvpn_entity::device::Entity::find_by_id(unique_id.to_string())
            .one(&self.db)
            .await?
        ...
        let mut device: modvpn_entity::device::ActiveModel = device.into();
