...
use modvpn_controller::auth::Auth;
use modvpn_migration::sea_orm::DatabaseConnection;
use modvpn_server::{ServerApi, ServerApiNoAuth};
use modvpn_types::modvpn_server::{AddDeviceRequest, UserCredentials};
...
impl DeviceService {
    ...
    async fn handle_sign_in_inner(
        ...
        let mut modvpn_service = ServerApiNoAuth::new().await?;
        self.device_storage
            .init()
            .await
        ...
        // make API call
        let add_device_response = modvpn_service.add_device(add_device_request).await?;
