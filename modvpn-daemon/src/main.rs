...
use modvpn_daemon::{create_daemon, logging, runtime::create_runtime, shutdown::ShutdownManager};
...
fn main() -> ExitCode {
    ...
    let exit_code = match runtime.block_on(run_modvpn(
        #[cfg(windows)]
        cli.service,
    )) {
...
#[cfg(windows)]
async fn run_modvpn(service: bool) -> Result<(), String> {
    use modvpn_daemon::system_service;
    if service {
        system_service::run()
    } else {
        run_modvpn_inner().await
    }
}

#[cfg(unix)]
async fn run_modvpn() -> Result<(), String> {
    run_modvpn_inner().await
}

async fn run_modvpn_inner() -> Result<(), String> {
