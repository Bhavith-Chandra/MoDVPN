...
pub type modvpnServiceClient<P> = proto::modvpn_service_client::modvpnServiceClient<AuthLayer<P>>;
pub type modvpnServiceNoAuthClient =
    proto::modvpn_service_client::modvpnServiceClient<tonic::transport::Channel>;

async fn create_channel() -> Result<tonic::transport::Channel, tonic::transport::Error> {
    let tls = ClientTlsConfig::new();

    let api_host_port = modvpn_config::config().grpc_api_host_port();
...
pub async fn new_modvpn_service_client<P: TokenProvider + 'static>(
    token_provider: P,
) -> Result<modvpnServiceClient<P>, tonic::transport::Error> {
    ...
    Ok(proto::modvpn_service_client::modvpnServiceClient::new(
        channel,
    ))
}

pub async fn new_modvpn_service_no_auth_client(
) -> Result<modvpnServiceNoAuthClient, tonic::transport::Error> {
    let channel = backoff::future::retry(backoff(), || async {
...
pub struct ServerApiNoAuth {
    client: modvpnServiceNoAuthClient,
}

impl ServerApiNoAuth {
    pub async fn new() -> Result<Self, tonic::transport::Error> {
        Ok(Self {
            client: new_modvpn_service_no_auth_client().await?,
        })
    }
